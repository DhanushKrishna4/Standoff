...
import { enrichCatalog, MOOD_AXES } from './enrich.mjs';
...
export function serialize(catalog) {
  ...
  const axes = '[\n' + MOOD_AXES.map(
    (a) => `  { key: '${a.key}', low: ${JSON.stringify(a.low)}, high: ${JSON.stringify(a.high)} },`
  ).join('\n') + '\n]';
  return `/* =============================================================================
...
async function main() {
  ...
  const balanced = balance(pool, TARGET).map(({ tmdbId, ...rest }) => rest);   // drop helper field
  const catalog = enrichCatalog(balanced);   // add learned tone axes, novelty, rating, warnings
